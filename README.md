# Claude使用量チェッカー Chrome拡張機能 v2.6.14

Claude.aiの使用量を簡単に確認できるChrome拡張機能です。

## v2.6の変更点

- 📝 **ドキュメント更新**: v2.5の変更内容をREADMEに反映

## v2.5の変更点

- 🖥️ **スプリット表示対応**: ウィジェットの位置を右端からの相対位置に変更
- ✨ **表示の安定性向上**: ブラウザのスプリット表示時でもウィジェットが見えなくなる問題を修正
- 🔧 **位置管理の改善**: 左端基準から右端基準に変更し、ウィンドウサイズ変更に対応

## v2.4の変更点

- ✅ **ウィジェット位置の自動調整**: 画面外に出た場合、自動的に表示可能な位置に調整
- ✅ **起動時の表示問題を修正**: 折りたたみ状態や位置の問題で見えないケースを改善
- ✅ **デバッグ情報の強化**: ウィジェットの状態をより詳細に記録
...
**v2.6** (2025-11-05)
- ドキュメント更新（v2.5の変更内容を反映）

**v2.5** (2025-11-05)
- スプリット表示対応（右端からの相対位置に変更）
- ブラウザのスプリット表示時でもウィジェットが見えなくなる問題を修正
- 位置管理の改善（左端基準→右端基準）
